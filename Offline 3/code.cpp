...
int32_t main()
{
    ...
    while(true)
    {
        ...
        else if(find(ALL(instructionsI),ins) != instructionsI.end()) /// I type
        {
            ...
            if(ins=="addi" || ins=="subi" || ins=="andi" || ins=="ori")
            {
                ...
                string t = dec_to_hex(add_imm);
                if((int)t.size()==1) t = "0"+t;
            ...
            else if(ins=="beq" || ins=="bneq")
            {
                ...
                {
                    output = MAP_INS_OPC[ins] + MAP_REG_ADD[one] + MAP_REG_ADD[other] + MAP_LABEL_ADD[label];
                }
            }
        ...
        {
            /// label
            vector <string> res = tokenize(ins,':');
            string label = res[0];
            DBG(label);
    ...
    for(auto &str:machine_code)
    {
        if(label_rem.find(str)!=label_rem.end())
        {
            ...
            else str += MAP_LABEL_ADD[label_rem[str]];
